...
# Import RandomOverSampler to balance dataset
...
def main():
    ...
    # Balance the dataset to improve sensitivity
    ros = RandomOverSampler(random_state=42)
    evidence_resampled, labels_resampled = ros.fit_resample(evidence, labels)

    # Split data into training and testing sets
    X_train, X_test, y_train, y_test = train_test_split(
        evidence_resampled, labels_resampled, test_size=TEST_SIZE, random_state=42, stratify=labels_resampled
    )
    ...
    with open(filename, encoding="utf-8") as f:
        ...
        for row in reader:
            try:
                ...
                raise Exception(f"Missing or unknown column: {e}")
